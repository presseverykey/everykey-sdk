...
void KeyboardInit(uint8_t* inBuffer, uint8_t* outBuffer, HidInReportHandler inReportHandler, HidOutReportHandler outReportHandler) {
	HIDInit(deviceDescriptor, configDescriptor, manufacturerName, deviceName, serialName, configDescriptor+0x12,
			reportDescriptor, sizeof(reportDescriptor), inBuffer, outBuffer, inReportHandler, outReportHandler);
}
