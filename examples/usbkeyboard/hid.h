...
/** called when a device-to-host report is requested. Implementations should fill outBuffer with current values.
 * @param reportType report type (1=input, 2=output, 3=feature)
 * @param reportId report id to fill. Currently, only one report (0) is supported. Hi byte contains report type.
...
/** initializes a HID device. For now, only simple HID devices are supported (one HID interface, no other interfaces, one report) 
 ...
 * @param hidDesc USB HID descriptor (may point to repective subpart of configuration descriptor)
