...
// `USB_Device_Definition` to be passed to `USB_Init`.	
...
USB_Device_Struct cdcDevice;

int counter;

void everycdc_init(everycdc *cdc) {
	cdc->everycdc_behaviour  = &cdcBehaviour;
	cdc->everycdc_device_def = &usbDefinition;
	cdc->everycdc_device     = &cdcDevice;

	USBCDC_ResetBehaviour(cdc->everycdc_behaviour);
	USB_Init(cdc->everycdc_device_def, cdc->everycdc_device);
	USB_SoftConnect(cdc->everycdc_device);
}

// read a byte from the cdc device, returns the value of the byte or -1
// if there is nothing to read.
int everycdc_read_byte(everycdc *cdc) {
	uint8_t byte;
	if (USBCDC_ReadBytes(cdc->everycdc_device, 
	                     cdc->everycdc_behaviour, 
											 &byte,
											 1)) 
	{
		return byte;
...
bool everycdc_write_byte(everycdc *cdc, uint8_t byte) {
	if (USBCDC_WriteBytes(cdc->everycdc_device, 
												cdc->everycdc_behaviour,
												&byte, 
												1))
	{
